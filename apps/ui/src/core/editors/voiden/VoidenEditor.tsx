...
// Parses a JSON string and re-serializes it with sorted keys, for stable
// comparison regardless of the original key order. Returns the input
// unchanged if it isn't valid JSON.
const canonicalizeJSONString = (value: unknown): unknown => {
    ...
    if (key === "attrs") {
      ...
      if (attrs && typeof attrs === "object" && !Array.isArray(attrs)) {
        ...
        for (const ak of Object.keys(attrs)) {
          // Drop uid, and drop any key whose value is undefined — a live
          // ProseMirror node materializes every schema-declared attr (even
          // ones defaulting to undefined), while raw parsed-from-disk JSON
          // only has keys actually present in the source. Without this, a
          // node whose only extra attr is one of these defaults (e.g.
          // gqlquery's importedFrom) ends up with a non-empty `stripped`
          // full of invisible-when-serialized undefined values on the live
          // side while the parsed side has none — same rendered value, but
          // a different "is attrs empty" verdict, so the two sides diverge.
          if (ak === "uid" || attrs[ak] === undefined) continue;
          // gqlvariables.body is regenerated by the GraphQL plugin in
          // query-declaration key order, which usually differs from
          // whatever order the source file's JSON happened to use (e.g. a
          // Postman-imported file just carries over Postman's own order).
          // Canonicalize so a key-order-only difference — which can persist
          // even after reverting the query edit that triggered a regen —
          // never shows as a false unsaved change.
          if (ak === "body" && obj.type === "gqlvariables") {
            stripped[ak] = canonicalizeJSONString(attrs[ak]);
            continue;
          }
          stripped[ak] = attrs[ak];
        }
        // Omit attrs entirely when empty — consistently on both sides now
        // that phantom undefined-valued keys no longer count as "present".
        if (Object.keys(stripped).length > 0) result[key] = stripped;
      }
    } else if (key === "content" && Array.isArray(obj[key]) && (obj[key] as unknown[]).length === 0) {
      // Omit an empty content array entirely rather than keeping `[]`. A live
      // ProseMirror node always serializes `content: []` for an empty node
      // (e.g. a blank paragraph), while parsed-from-disk JSON for a node
      // that was never given any content simply never has the key — same
      // node, different "has content" verdict, so the two sides diverge.
      continue;
    } else {
