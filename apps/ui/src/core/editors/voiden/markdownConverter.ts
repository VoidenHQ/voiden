...
import { blockOwnership } from "./extensions/PlaceholderBlock";

// packageJson here is apps/ui's own internal sub-package version (not kept in
// sync with real releases) — not what should be stamped into files. The
// actual Voiden app version (same one shown in the About modal/Settings
// screen) only comes from the main process via IPC, which is async, but this
// module's callers (the save path) are synchronous. Fetch it once and cache
// it; `packageJson.version` is only a fallback for the brief window before
// the IPC call resolves (or in non-Electron/test environments).
let cachedAppVersion: string | undefined;
if (typeof window !== "undefined" && window.electron?.getVersion) {
  window.electron.getVersion().then((v: string) => {
    if (v) cachedAppVersion = v;
  }).catch(() => {});
}
...
export function addVersionFrontmatter(
  ...
  const frontmatterObj = {
    version: options?.appVersion ?? cachedAppVersion ?? appVersion,
...
const fallback = (state: MarkdownSerializerState, node: Node) => {
  ...
  // Stamp plugin-owned blocks with who created them and which version — recomputed
  // fresh on every save (like the file-level appVersion frontmatter) so it always
  // reflects the currently-installed plugin, not a live schema attr tracked in history.
  const owner = blockOwnership.get(node.type.name);
  if (owner) {
    nodeJson = { ...nodeJson, attrs: { ...nodeJson.attrs, pluginId: owner.pluginId, pluginVersion: owner.pluginVersion } };
  }
...
// missingPluginBlock stands in for a block whose owning plugin isn't installed
// at all — its schema/content shape is unknown, so instead of round-tripping
// through node.toJSON() (which would permanently rewrite the fence's `type` to
// "missingPluginBlock" and lose the original block forever), write back the
// exact original fenced text captured in `rawText`. Once the plugin is
// installed, reparsing that unchanged text resolves to the real node again.
const missingPluginBlockSerializer = (state: MarkdownSerializerState, node: Node) => {
  const rawText = node.attrs?.rawText || "";
  state.text("```void\n" + rawText.replace(/\n?$/, "\n") + "```\n", false);
};
...
const buildNodeSerializers = (schema: any) => {
  ...
  Object.keys(schema.nodes).forEach((nodeName) => {
    ...
    } else if (nodeName === "missingPluginBlock") {
      serializers[nodeName] = missingPluginBlockSerializer;
    } else {
...
const processCubeBlockText = (text: string, schema: any): JSONContent => {
  ...
  // unless it's a tagged plugin block (pluginId set), in which case show a
  // dedicated "plugin not installed" card that preserves the raw text so it
  // resolves back to the real node once the plugin is installed.
  if (!schema.nodes[nodeJson.type]) {
    if (nodeJson.attrs?.pluginId && schema.nodes["missingPluginBlock"]) {
      return {
        type: "missingPluginBlock",
        attrs: {
          pluginId: nodeJson.attrs.pluginId,
          pluginVersion: nodeJson.attrs.pluginVersion ?? null,
          blockType: nodeJson.type,
          rawText: rawCubeText,
        },
      };
    }
