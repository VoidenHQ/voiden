...
describe("losslessJson", () => {
  ...
  it("preserves nested large integers from issue #395 fixture", () => {
    const json = '{"args":{"id":174322306148984899}}';
    const parsed = parseJsonLossless(json) as { args: { id: bigint } };

    expect(parsed.args.id).toBe(174322306148984899n);
    expect(losslessValueToString(parsed.args.id)).toBe("174322306148984899");
  });
});
