...
let
  ...
  cacheDrv = stdenv.mkDerivation {
    ...
    # hash only ever matches one of them. This was already fixed once (see
    # a6d506d) but yarn-project.nix is regenerated by `yarn install` and
    # doesn't know about this split, so a later lockfile change silently
    # collapsed it back to one shared value — this is that fix reapplied
    # against the current yarn.lock's real per-system hashes (pulled from
    # the "got:" side of the resulting "hash mismatch in fixed-output
    # derivation" error on each platform). To add a new system: build once
    # with any placeholder value here, take the "got:" hash from the error,
    # and add it below.
    outputHash = {
      "aarch64-darwin" = "sha512-RzEwxu1Tc00nBVXaffSHjDn3O9NzxpI7VSHMjLpE92oQA/z0ysvxOAj1uyyQi2ltI/ry42yqQYUZFWaQ0o+BGw==";
